...
async function main() {
  ...
  if (pullRequest.user.login !== allowedAuthor) {
    core.info(`Found PR author ${pullRequest.user.login} but only ${allowedAuthor} is allowed to enable automerge via this action; ending workflow.`);
    return;
  }
  ...
  core.info(`Automerge for #${pullRequest.number} \u201C${pullRequest.title}\u201D by ${pullRequest.user.login} enabled at ${enabledAt}`);
}
try {
  main();
} catch (error2) {
